    ...
    try:
        if a.transfer is None:
            ...
            for index,(start,end) in zip(a.transfer,ranges):
                ...
                if not wait_native_payload(win32clipboard,seq,pump=pythoncom.PumpWaitingMessages,timeout=10.0)['ready']:raise RuntimeError('CLIPBOARD')
            ...
            if not wait_native_payload(win32clipboard,seq,pump=pythoncom.PumpWaitingMessages,timeout=10.0)['ready']:raise RuntimeError('MOVE_CLIPBOARD')
            hwp.MoveDocEnd()
