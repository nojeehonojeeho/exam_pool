...
주 본문에 문제 1번부터 마지막 문항까지를 원본 순서대로 먼저 배치하고, 정답·풀이
문단과 해설 제목·그림은 주 본문에 넣지 않는다. 각 문제에는 실제 native endnote
reference를 하나만 연결하고, 미주 영역에는 대응하는 정답·해설만 같은 순서로
둔다. 평문 `[해설]`·`※` 표식이나 해설 페이지를 주 본문 뒤에 단순 병합하는 방식은
native 미주로 인정하지 않는다.
...
렌더에서 마지막 문제 다음에 미주 영역만 이어지는지, 문제 하나를 복사·이동할 때
native 미주 연결이 보존되는지를 확인한 뒤에만 통합본을 출고한다. 이 규칙은
