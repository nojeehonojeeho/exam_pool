...
출고 파일에는 검수용 내부 진단 문구를 남기지 않는다. 예를 들어
`문항과 무관한 스캔 잔여점으로 semantic 내용 없음.` 같은 감사·OCR 메타 문구가
HWP/HWPX 본문, 미주, 표·그림 캡션 또는 렌더 결과에 한 글자라도 나타나면
`INTERNAL_DIAGNOSTIC_TEXT_LEAK`로 즉시 차단한다. 진단 문구를 단순 삭제하여
문단 경계를 훼손해서는 안 되며, 재조판 시 문항·문단 구조를 보존하는 방식으로
제거한 뒤 HWPX XML, COM 재열림, 렌더에서 다시 확인한다. 이 검사는
`app.hwp_delivery_reaudit`의 금지문구 목록과 동일하게 유지하고, 새 금지문구는
코드·fixture·작업지시서를 함께 갱신한다.
