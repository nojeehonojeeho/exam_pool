...
PDF/HWP 변환 단계의 공통 실행 기본값은
[사용자 의도 기반 실행 기본값](USER_INTENT_EXECUTION_DEFAULT.md)을 따른다. 실행 의도가
명확하면 계획만 남기지 않고 안전한 단계부터 계속 진행한다.
...
## 4-1. 조판 규칙 (2026-08-03 실측으로 확정)

- 사진 칸은 export 가 `\파일이름\` 으로 감싸 내보낸다. 맨 파일명이면 글자가 그대로 박힌다.
- 합답형은 **학교합답0/1/2사진5선지** 를 쓴다. 옛 `합답형*3선지` 는 조각에 발문이
  박혀 있어 부정발문을 표현할 수 없다.
- 그림은 PNG 의 pHYs(5E 가 새기는 300dpi) 크기로 들어간다. 셀 안이면 셀에 맞춰진다.
- 시험지 판형(2단)은 `python -m hwp_palette.cli --markdown-file 세트.md --exam-page`.
  단 폭은 hwpPalette 스펙의 `column_width_mm` 에 맞춰 여백이 역산된다.
